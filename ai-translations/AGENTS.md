...
- The post-translation publish action must include only completed records that were actually updated and whose models have `draft_mode_active`; CMA bulk-publish requests must contain no more than 200 records each.

## Translation Workflow Notes
- Supported providers are currently OpenAI, Gemini, Anthropic, DeepL, and Yandex Translate.
