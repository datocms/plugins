...
- **Cancelling does not roll back records already written.** Stopping the run leaves the records translated so far in place.
- **You can resume an interrupted run.** Each bulk run is checkpointed to your browser (IndexedDB) after every record. If a run is cancelled — or the tab is closed mid-run — the next time you start a bulk translation the plugin offers to **Resume** it, re-translating only the records and locales that had not finished and leaving the completed ones untouched. It offers this only while the earlier run's field/locale rules still match your current configuration (change a translation rule and the stale run is not offered); a fully-completed run is dropped automatically. Choose **Start over** to discard the checkpoint and translate everything fresh.
...
- **Single field / whole record** (sidebar and field dropdown): a value with a content-corrupting error (truncation, a lost placeholder, structural drift, or a length-validator violation) is **withheld** — it is never staged into the form, so the existing value is kept and you cannot accidentally save corrupt content — and a blocking alert names what was not applied. Softer warnings still stage the value and show a notice so you can review before saving. (This mirrors the bulk write, which drops the same defective locales from the CMA update.)
