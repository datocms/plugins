...
export interface BuildTranslatedUpdatePayloadResult {
  ...
  /** Genuine per-field translation failures (not reference-copy notices). */
  warnings: string[];
}
...
export async function translateAndUpdateRecords(
  ...
  function reportTranslationResult(
    ...
    // summary line (if any) followed by any genuine per-field failures. These
    // ride on the progress update separately from `message`, so the UI can flag
    // the record (icon + tooltip) instead of concatenating a wall of text.
