...
- 0.7.2: Fixed Vercel cron deployments by using Vercel's authenticated GET convention with `CRON_SECRET`. Missed backup cadences now catch up on the next successful daily run, and overdue entries are clearly shown as "Due now" in the backup status overview.
