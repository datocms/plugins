...
import { findBlockNodesInDast, isStructuredTextValue } from './dast';
 ...
 */
export function getBlockTypeId(
  ...
  const relationships = block.relationships as
    | Record<string, unknown>
    | undefined;
  if (relationships) {
    const itemTypeRel = relationships.item_type as
      | Record<string, unknown>
      | undefined;
    if (itemTypeRel) {
 ...
 */
function extractBlocksFromStructuredTextValue(
  fieldValue: unknown,
): DastBlockRecord[] {
