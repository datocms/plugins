...
import { buildSchemaDiff } from '../diff/schema';
...
import type { CompareTaskContext, FilterValue, SchemaDiffRow, SchemaEntityType, SummaryRow, TableColumn } from '../types';

type Props = {
  ctx: RenderPageCtx;
};

const GROUPS: SchemaEntityType[] = ['model', 'block', 'fieldset', 'field'];

const COLUMNS: TableColumn<SchemaDiffRow>[] = [
  {
    key: 'name',
    title: 'Entity',
    render: (row) => (
      <div>
        ...
        {row.secondaryLabel ? <div>{row.secondaryLabel}</div> : null}
      </div>
    ),
  },
  {
...
function buildSummaryRows(summary: Record<SchemaEntityType, SummaryRow['counts']> | undefined) {
...
export default function SchemaDiffsPage({ ctx }: Props) {
  const { query, updateQuery } = usePageQueryState(ctx, 'schema-diffs');
  ...
  const activeGroup: SchemaEntityType = GROUPS.includes(query.entityType as SchemaEntityType)
    ? (query.entityType as SchemaEntityType)
    : 'model';
  ...
  const compare = useCompareTask(
    resolvedPair
      ? `schema-diffs:${resolvedPair.leftEnv}:${resolvedPair.rightEnv}`
      : 'schema-diffs:inactive',
    Boolean(resolvedPair && ctx.currentUserAccessToken),
    load,
  );

  const summaryRows = useMemo(
    () => buildSummaryRows(compare.data?.summary),
  ...
  const handleFilterChange = async (filter: FilterValue) => {
    await updateQuery({ filter, entityId: undefined });
  };
  ...
  const results = compare.status === 'loading' && !compare.data ? (
    ...
    <StateView title="Could not load schema diff" message={compare.error ?? 'Unknown error'} tone="error" />
  ) : filteredRows.length === 0 ? (
    <StateView title="No matching rows" message="Try another status filter or entity group." />
  ) : (
    <ResultsTable
      rows={filteredRows}
      columns={COLUMNS}
  ...
  const summary = (
    <>
      ...
      />
      <StatusFilter value={query.filter} onChange={handleFilterChange} />
    </>
  );
  ...
  if (environments.isLoading && environments.environmentIds.length === 0) {
    return (
      <PageShell
        ctx={ctx}
        title="Schema diffs"
        description="Compare models, blocks, fieldsets, and fields between two environments."
        toolbar={<div />}
        summary={<div />}
  ...
  return (
    <PageShell
      ctx={ctx}
      title="Schema diffs"
      description="Compare models, blocks, fieldsets, and field configuration between two environments in the same project."
      toolbar={
        <EnvironmentToolbar
