...
export default function FieldExtension({ ctx }: FieldExtensionProps) {
  ...
  const mainLocale = isMultiLocale ? availableLocales[0] : '';
  ...
  const copyToAllLocales = useCallback(async () => {
    if (!isMultiLocale || !Array.isArray(availableLocales)) {
      return;
    }
    ...
    const otherLocales = availableLocales.slice(1);

    await Promise.all(
      otherLocales.map((locale) =>
        ctx.setFieldValue(
          `${ctx.field.attributes.api_key}.${locale}`,
          removeBlockItemIdsImmutable(mainLocaleValue),
        ),
      ),
    );
    ctx.notice('Value copied to all locales');
  }, [ctx, mainLocale, availableLocales, isMultiLocale]);
  ...
  return (
    <ErrorBoundary ctx={ctx}>
      <Canvas ctx={ctx}>
        {isAtMainLocale && (
          <Button onClick={copyToAllLocales} buttonType="muted" buttonSize="s">
            Copy to all locales
          </Button>
        )}
        {!isAtMainLocale && (
          <Button
            onClick={copyFromMainLocale}
            buttonType="muted"
            buttonSize="s"
          >
