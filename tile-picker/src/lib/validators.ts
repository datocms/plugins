...
const validateOption = (data: unknown, index: number): Result => {
	...
	for (const field of ['name', 'type', 'value']) {
		if (d[field] === undefined) {
			return error(lang(EN_OPTION_MISSING_FIELD, {index: `${index}`, field}));
		}
		if (!isString(d[field])) {
			return error(lang(EN_OPTION_NON_STRING_FIELD, {index: `${index}`, field}));
		}
		if (field !== 'type' && !(d[field] as string).trim()) {
			return error(lang(EN_OPTION_MISSING_FIELD, {index: `${index}`, field}));
		}
	}

	if (isString(d.type) && !VALID_OPTION_TYPES.includes(d.type)) {
		return error(lang(EN_OPTION_INVALID_TYPE, {index: `${index}`, type: d.type as string}));
	}

	const sourceField = d.type === 'image' ? 'url' : 'color';
	if (!d[sourceField] || !(d[sourceField] as string).trim()) {
		return error(lang(EN_OPTION_MISSING_FIELD, {index: `${index}`, field: sourceField}));
	}
	if (!isString(d[sourceField])) {
		return error(lang(EN_OPTION_NON_STRING_FIELD, {index: `${index}`, field: sourceField}));
	}
...
const validatePresetsConfig = (data: unknown): Result => {
	...
	for (const [i, key] of Object.keys(d).entries()) {
		const preset = d[key];
		if (!isArray(preset)) {
...
const validateFieldConfig = (data: unknown): Result => {
	...
	if (d.presets !== undefined && !isArray(d.presets)) {
		return error(lang(EN_FIELD_IS_NOT_ARRAY, {field: 'Presets'}));
	}
	if (isArray(d.presets) && !d.presets.every(isString)) {
		return error(lang(EN_FIELD_IS_NOT_STRING_ARRAY, {field: 'Presets'}));
	}
	...
	if (d.presentation !== undefined && !Object.keys(d.presentation as object).every(k => VALID_PRESENTATION_KEYS.includes(k))) {
